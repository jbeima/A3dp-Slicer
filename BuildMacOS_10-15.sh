...
while getopts ":iaxbh" opt; do
  case ${opt} in
    ...
    b )
        ...
        ;;
    h ) echo "Usage: ./BuildMacOS_10-15.sh [-i]"
...
echo -n "[7/9] Configuring Slic3r..."
{
    ...
    # cmake
    pushd build
    echo "Cmake command: cmake .. -DCMAKE_PREFIX_PATH=\"$PWD/../deps/build/destdir/usr/local\" -DCMAKE_OSX_DEPLOYMENT_TARGET=\"10.15\" -DSLIC3R_STATIC=1 ${BUILD_ARGS}"
