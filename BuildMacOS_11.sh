...
while getopts ":iaxbh" opt; do
  case ${opt} in
    ...
    b )
        ...
        ;;
    h ) echo "Usage: ./BuildMacOS_11.sh [-i]"
...
echo -n "[3/9] Configuring dependencies..."
{
    ...
    # cmake deps
    echo "Cmake command: cmake .. -DCMAKE_OSX_DEPLOYMENT_TARGET=\"11\" ${BUILD_ARCH} "
    pushd deps/build
    cmake .. -DCMAKE_OSX_DEPLOYMENT_TARGET="11" $BUILD_ARGS 
    echo "ls deps/build:"
    ls -al
...
echo -n "[7/9] Configuring Slic3r..."
{
    ...
    # cmake
    pushd build
    echo "Cmake command: cmake .. -DCMAKE_PREFIX_PATH=\"$PWD/../deps/build/destdir/usr/local\" -DCMAKE_OSX_DEPLOYMENT_TARGET=\"11\" -DSLIC3R_STATIC=1 ${BUILD_ARGS}"
    cmake .. -DCMAKE_PREFIX_PATH="$PWD/../deps/build/destdir/usr/local" -DCMAKE_OSX_DEPLOYMENT_TARGET="11" -DSLIC3R_STATIC=1 ${BUILD_ARGS}
} #&> $ROOT/build/Build.log # Capture all command output
echo "done"
